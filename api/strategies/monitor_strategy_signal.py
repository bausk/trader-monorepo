...
def calculate_signal(primary_data: List[PricepointSchema], secondary_data: List[PricepointSchema]) -> SignalResultSchema:
    ...
    arbitrage_difference = ((primary_dataframe['price'] * coefficients) - secondary_dataframe['price'])
    ...
    weighted_arbitrage_indicator = arbitrage_indicator.rolling(rolling_indicator_window).sum()
    sell_difference = ((primary_dataframe['price'] * coefficients) - secondary_dataframe['price'])
